...
export async function POST(r: Request) { try { const {nama,kontak,alamat}=await r.json(); const e=validateRequired({nama}); if(e) return apiBadRequest(e); return apiCreated(await createSupplier(nama,kontak||'',alamat||'')); } catch { return apiServerError('POST /api/supplier'); } }
export async function PUT(r: Request) { try { const {id,nama,kontak,alamat}=await r.json(); const e=validateRequired({id,nama}); if(e) return apiBadRequest(e); const i=await updateSupplier(id,nama,kontak||'',alamat||''); if(!i) return apiNotFound(); return apiSuccess(i); } catch { return apiServerError('PUT /api/supplier'); } }
